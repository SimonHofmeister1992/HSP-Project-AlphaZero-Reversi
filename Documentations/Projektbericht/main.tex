...
Des Weiteren beinhaltet das Paket die Definition spielrelevanter und klassenübergreifender Konstanten, sowie der Kommandozeilenparameter. Die verfügbaren fünf Kommandozeilenparameter sind in Tabelle \ref{tab:cmd_params} dargestellt. Die Parameter q, l und m sind vom Typ boolean und werden entweder angegeben oder weggelassen. Der Parameter q bewirkt, dass nur die nötigsten Informationen ausgegeben werden, während der Parameter l veranlasst, dass die KI nach jedem Spiel nachtrainiert. Bei der Angabe des Parameters m wird der Client ohne die Verwendung des Neuronalen Netzes ausgeführt. Die restlichen drei Parameter müssen in der Regel nicht angegeben werden, da die KI üblicherweise am selben Rechner wie der Server gestartet wird, zudem kann der Server mit mehreren identischen Gruppennummern von umgehen, dieser ist reine Kosmetik um die Clients als Mensch differenziert beim Spiel beobachten zu können.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ | l | X | l | }
		...
		l & Der Client trainiert nach dem Spiel &  \\ \hline
		m & Der MCTS wird ohne Neuronales Netz verwendet. & \\ \hline
	\end{tabularx}
\end{table}
...
\begin{flalign*}
& \hspace{-5.3 cm} \text{sudo apt-get update} \\
& \hspace{-5.3 cm} \text{sudo apt-get upgrade} \\
& \hspace{-5.3 cm} \text{sudo apt-get autoremove} \\
\end{flalign*}
...
& \hspace{3.4 cm}\text{sudo mv cuda-ubuntu1804.pin /etc/apt/preferences.d/cuda-repository} \\
& \hspace{4.1 cm}\text{-pin-600} \\
& \hspace{3.4 cm}\text{wget http://developer.download.nvidia.com/compute/cuda/10.2/Prod} \\
& \hspace{4.1 cm}\text{/local\_installers/cuda-repo-ubuntu1804-10-2-local-10.2.89-440.33.01} \\
& \hspace{4.1 cm}\text{\_1.0-1\_amd64.deb} \\
& \hspace{3.4 cm}\text{sudo dpkg -i cuda-repo-ubuntu1804-10-2-local-10.2.89-440.33.01\_1.0-1} \\
& \hspace{4.1 cm}\text{\_amd64.deb} \\
& \hspace{3.4 cm}\text{sudo apt-key add /var/cuda-repo-10-2-local-10.2.89-440.33.01/7fa2af80} \\
& \hspace{4.1 cm}\text{.pub} \\
& \hspace{3.4 cm}\text{sudo apt-get update} \\
& \hspace{3.4 cm}\text{sudo apt-get install nvidia-440 // alternativ kleinere Treiberversion,} \\
& \hspace{4.1 cm}\text{Anpassung an die DL4J kompatiblen Cuda-Versionen siehe Tab. \ref{tab:cuda-cudnn},} \\
& \hspace{4.1 cm}\text{Anpassung in der pom.xml des Reversi-Clients} \\
& \hspace{3.4 cm}\text{sudo apt-get update} \\
& \hspace{3.4 cm}\text{// Logout und erneuten Login durchführen.} \\
& \hspace{3.4 cm}\text{sudo apt-get -o Dpkg::Options::=\grqq{}--force-overwrite\grqq{} install cuda cuda-} \\
& \hspace{4.1 cm}\text{drivers} \\
\end{flalign*}
\begin{flalign*}
& \hspace{3.8 cm}\text{sudo vi /etc/profile} \\
...
\begin{flalign*}
& \hspace{-4.4 cm}\text{make clean \&\& make} \\
\end{flalign*}
...
\begin{flalign*}
& \hspace{-0.6 cm} \text{sudo apt-get update} \\
& \hspace{-0.6 cm} \text{sudo apt-get upgrade} \\
& \hspace{-0.6 cm} \text{sudo apt-get autoremove} \\
& \hspace{-0.6 cm} \text{// Logout und erneuten Login durchführen.} \\
\end{flalign*}
