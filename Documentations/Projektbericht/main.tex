...
%TODO Quelle https://medium.com/oracledevs/lessons-from-alphazero-part-3-parameter-tweaking-4dceb78ed1e5
...
& \hspace{3.2 cm}\text{// Bei kleinerer GCC-Version: } \\
& \hspace{3.2 cm}\text{sudo apt-get install gcc} \\
& \hspace{3.2 cm}\text{gcc --version // erneute Prüfung} \\
\end{flalign*}
...
\begin{flalign*}
& \hspace{3.8 cm}\text{sudo add-apt-repository ppa:graphics-drivers/ppa} \\
& \hspace{3.8 cm}\text{sudo apt-key adv --fetch-keys} \\ 
& \hspace{4.5 cm}\text{http://developer.download.nvidia.com/compute/cuda/repos/} \\
& \hspace{4.5 cm}\text{ubuntu1804/x86\_64/7fa2af80.pub} \\
& \hspace{3.8 cm}\text{echo \grqq{}deb https://developer.download.nvidia.com/compute/cuda/} \\
& \hspace{4.5 cm}\text{repos/ubuntu1804/x86\_64 /\grqq{} | sudo tee /etc/apt/sources.list.d/} \\
& \hspace{4.5 cm}\text{cuda.list} \\
& \hspace{3.8 cm}\text{wget https://developer.download.nvidia.com/compute/cuda/repos/} \\
& \hspace{4.5 cm}\text{ubuntu1804/x86\_64/cuda-ubuntu1804.pin} \\
...
& \hspace{4.1 cm}\text{sudo apt-get update} \\
& \hspace{4.1 cm}\text{sudo apt-get install nvidia-440 // alternativ kleinere Treiberversion,} \\
& \hspace{4.8 cm}\text{Anpassung an die DL4J kompatiblen Cuda-Versionen siehe Tab. \ref{tab:cuda-cudnn},} \\
& \hspace{4.8 cm}\text{Anpassung in der pom.xml des Reversi-Clients} \\
& \hspace{4.1 cm}\text{sudo apt-get update} \\
& \hspace{4.1 cm}\text{// Logout und erneuten Login durchführen.} \\
& \hspace{4.1 cm}\text{sudo apt-get -o Dpkg::Options::=\grqq{}--force-overwrite\grqq{} install cuda cuda-} \\
& \hspace{4.8 cm}\text{drivers} \\
& \hspace{4.1 cm}\text{sudo vi /etc/profile} \\
& \hspace{5.1 cm}\text{Um folgenden Inhalt erweitern:} \\
& \hspace{5.1 cm}\text{export PATH=/usr/local/cuda-$<$cuda-version$>$/bin:\$PATH} \\
& \hspace{5.1 cm}\text{export LD\_LIBRARY\_PATH=/usr/local/cuda-$<$cuda-version$>$/} \\
& \hspace{5.1 cm}\text{lib64:\$LD\_LIBRARY\_PATH} \\
& \hspace{5.1 cm}\text{export CUDA\_PATH=/usr/local/cuda-$<$cuda-version$>$} \\
& \hspace{4.1 cm}\text{// Logout und erneuten Login durchführen.} \\
& \hspace{4.1 cm}\text{sudo ldconfig} \\
& \hspace{4.1 cm}\text{sudo apt-get -o Dpkg::Options::="--force-overwrite" install nvidia-cuda-} \\
& \hspace{4.8 cm}\text{toolkit} \\
\end{flalign*}

Falls es gewünscht ist, die Rechenzeit bestmöglich zu optimieren, kann zusätzlich das CuDNN Paket installiert werden.
Im folgenden wird CuDNN 7.6.5 als kompatible Version zu Cuda 10.2 verwendet, siehe Tab. \ref{tab:cuda-cudnn}. Für den Download von CuDNN muss man sich kostenlos für das Cuda-Developer-Programm anmelden.
