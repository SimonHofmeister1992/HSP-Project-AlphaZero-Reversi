...
class NeuronalNet {
    ...
    private BlockOutput addConvBlock(ComputationGraphConfiguration.GraphBuilder graphBuilder, String input, int numOfInputPlanes, int[] kernelSize, int[] stride, ConvolutionMode cMode) {
        ...
        graphBuilder.addLayer(activation, new ActivationLayer.Builder().activation(Activation.RELU).build(), batchNorm);
    ...
    private BlockOutput addResidualLayers(ComputationGraphConfiguration.GraphBuilder graphBuilder, String nameLastLayer, int[] kernelSize, int[] stride, ConvolutionMode cMode) {
        ...
        for(int i = 0; i < 40; i++){
            ...
            graphBuilder.addLayer(activation1, new ActivationLayer.Builder().activation(Activation.RELU).build(), batchNorm1);
            ...
            graphBuilder.addLayer(activation, new ActivationLayer.Builder().activation(Activation.RELU).build(), mergeBlock);
